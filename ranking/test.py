...
def linear_ranking(query, filtered_docs_ids, index_title) -> dict:
    """Rank documents"""
    ...
    # somme des scores
    scores = {docId: score_counts[docId]+score_o[docId] for docId in filtered_docs_ids}

    for key, val in scores.items():
        print(key, val)

def linear_ranking(query, filtered_docs, index, weights):
    """List of ranked documents based on the linear ranking score."""
    ...
    tokenized_query = [token for token in tokenize(query) if token in index_title.keys()]
    tokenized_query = list(set(tokenized_query))
...
with open('content_pos_index.json') as f:
    index_content = json.load(f)

# initializing the query
query = "pourquoi erreur 404"
...
# rank documents using custom linear ranking with custom weights for each score
weights = {"num_tokens": 0.5, "importance": 1.0}
ranking = linear_ranking(query, filtered_docs, index_title, weights)

print(ranking)

# format ranking string for JSON export and save ranking
formated_ranking = format_ranking_results(documents, ranking)
save_json(formated_ranking, 'results.json')